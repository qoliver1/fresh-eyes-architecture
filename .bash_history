curl -fsSL https://hermes-agent.nousresearch.com/install.sh | bash
ls
cd
cs
ls
help
dirs
clear
$ curl -fsSL https://hermes-agent.nousresearch.com/install.sh | bash
ls
curl -fsSL https://hermes-agent.nousresearch.com/install.sh | bash
# 1. Force a complete update of your Termux package lists and core libraries
pkg update && pkg upgrade -y
node -v
curl -fsSL https://hermes-agent.nousresearch.com/install.sh | bash
hermes
hermes --resume 20260616_222956_25f9d6
exit
hermes gateway run
/clear
clear
hermes gateway run
hermes gateway run --verbose
pkg install build-essential python-dev -y
exit
hermes gateway run
hermes gateway run --verbose
pkg install build-essential python-dev -y
hermes gateway run -v
hermes
clear
hermes gateway run -v
hermes gateway setup
hermes gateway
exit
clear
hermes
exit
hermes
pgrep sshd
sshd
pkillsshd
pkill sshd
hermes
/exit
exit
nano $PREFIX/etc/ssh/sshd_config
sshd
sshd --help
sshd
exit
clear
hermes
exit
hermes dashboard
clear
hermes
hermes gateway setup
nohup hermes gateway run >~/.hermes/logs/gateway.log 2>&1 &
hermes gateway run
hermes gateway run -v
hermes gateway run
hermes gateway run -v
pkg install clang make python-pip -y
pip install brotlicffi
hermes gateway run -v
pkg install libopus libsodium -y
pip install "discord.py[voice]==2.7.1"
pip install "discord.py[voice]==2.7.1" -v
hermes gateway run -v
hermes gateway restart
sshd
stop
exit
hermes
hermes gateway setup
hermes
hermeHermes
hermes
clear
hermes
clear
hermesHermes
hermes
clear
exit
clear
hermes
clear
hermes
clear
hermes
clear
hermes
clear
hermes
clear
hermes
clear
hermes
clear
hermes
clear
hermes
clear
hermes
hermes --resume 20260628_215000_1f772e
hermes
yes
hermes chat --model "gemma-4-26b-a4b-it"
hermes
